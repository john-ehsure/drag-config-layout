...
export default {
  ...
  beforeDestroy: function() {
    ...
    removeEvent(document.documentElement, 'mousedown', this.checkParentSize)
  ...
  methods: {
    ...
    changeWidth(val) {
      const [newWidth, newHeight] = snapToGrid(this.grid, val, 0, this.scale)
      ...
      this.right = right
      this.bottom = bottom
      this.width = this.windowResizeX ? newWidth : width

      if (this.windowResizeX) {
        this.windowResizeX = false
      }
    },
    changeHeight(val) {
      const [newWidth, newHeight] = snapToGrid(this.grid, 0, val, this.scale)
      ...
      this.right = right
      this.bottom = bottom
      this.height = this.windowResizeY ? newHeight : height
  ...
  watch: {
    canvasNumber: {
      handler: function(newVal, oldVal) {
        if (newVal !== oldVal) {
          addEvent(document.documentElement, 'mousedown', this.checkParentSize)
        }
      },
      deep: true
    },
